...
import { ArrowUpRight, ChevronLeft, ChevronRight, Download, ExternalLink, LoaderCircle, Search, Trash2 } from "lucide-react";
...
export default function OperationsDashboard() {
  ...
  const exportCsv = trpc.assessment.exportCsv.useMutation({
    onSuccess: (rows) => {
      const columns = ["ID", "Status", "Organisation", "Contact", "Email", "Phone", "Postcode", "Assets", "Quantity", "Collection timing", "Data requirement", "Inventory", "On-site erasure", "Submitted", "Retention review", "Notes"];
      const lines = rows.map((row) => [row.id, row.status, row.organisation, row.fullName, row.email, row.phone, row.sitePostcode, row.assetCategories, row.approximateAssetCount, row.collectionTimeline, row.dataSecurityRequirement, row.hasInventory ? "Yes" : "No", row.requiresOnSiteErasure ? "Yes" : "No", row.createdAt.toISOString(), row.retentionReviewAt?.toISOString(), row.notes].map(csvCell).join(","));
      const blob = new Blob([[columns.map(csvCell).join(","), ...lines].join("\n")], { type: "text/csv;charset=utf-8" });
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `reborn-assessments-${new Date().toISOString().slice(0, 10)}.csv`;
      link.click();
      URL.revokeObjectURL(url);
      toast(`${rows.length} assessment${rows.length === 1 ? "" : "s"} exported`);
    },
    onError: (error) => toast("CSV export could not be created", { description: error.message }),
  });

  const setFilter = (next: Status | "all") => { setStatus(next); setPage(1); };
  const totalPages = Math.max(1, Math.ceil((enquiries.data?.total ?? 0) / 12));
  const downloadCsv = () => exportCsv.mutate({ search: search || undefined, status: status === "all" ? undefined : status, sort });
