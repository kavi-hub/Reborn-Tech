...
export default function BulkItadDash() {
  ...
  return <DashboardLayout><div className="bulk-dash">
    ...
    <ClientAccountControls brand="bulk_gsm" workspace="Bulk GSM ITAD Dash" />
    <section className="bulk-job-table"><div className="bulk-job-table-head"><p className="ops-kicker">BULK GSM CORE JOBS</p><span>{jobs.data?.length ?? 0} active routes</span></div>{jobs.isLoading ? <div className="ops-loading"><LoaderCircle className="assessment-spin" size={20} />Loading Bulk GSM jobs</div> : jobs.data?.length ? <div>{jobs.data.map((row) => <button type="button" key={row.collection.id} className={`bulk-job-row ${selectedJobId === row.job?.id ? "is-active" : ""}`} onClick={() => row.job && setSelectedJobId(row.job.id)}><div><span>{row.collection.reference}</span><strong>{row.collection.title}</strong><small>{row.organisation.name} · {row.collection.collectionPostcode || "Postcode not stated"}</small></div><div className="bulk-stage"><span>CORE STAGE</span><strong>{row.job ? stageLabel(row.job.stage) : stageLabel(row.collection.status)}</strong></div></button>)}</div> : <div className="ops-empty"><p>No Bulk GSM Core jobs yet.</p><span>Open the first job to begin its secure ITAD route.</span></div>}</section>
    {(() => { const selected = jobs.data?.find((row) => row.job?.id === selectedJobId) ?? jobs.data?.find((row) => row.job); return selected?.job ? <CoreJobRecordsPanel jobId={selected.job.id} brand="bulk_gsm" workspace="Bulk GSM" /> : null; })()}
  </div></DashboardLayout>;
}
