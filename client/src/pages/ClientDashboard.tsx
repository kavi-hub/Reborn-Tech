...
const downloadZip = (contentBase64: string, fileName: string) => { const bytes = Uint8Array.from(atob(contentBase64), (character) => character.charCodeAt(0)); const href = URL.createObjectURL(new Blob([bytes], { type: "application/zip" })); const anchor = document.createElement("a"); anchor.href = href; anchor.download = fileName; anchor.click(); URL.revokeObjectURL(href); };
...
export default function ClientDashboard() {
  ...
  const [selectedArchiveJobIds, setSelectedArchiveJobIds] = useState<Set<number>>(() => new Set());
  ...
  const bulkExportAudit = trpc.clientPortal.bulkExportAudit.useQuery(undefined, { enabled: Boolean(session.data && session.data.role === "admin") });
  ...
  const bulkCompletionExport = trpc.clientPortal.bulkDownloadCompletionSummaries.useMutation({ onSuccess: ({ contentBase64, fileName }) => { downloadZip(contentBase64, fileName); toast("Selected completion summaries exported"); setSelectedArchiveJobIds(new Set()); }, onError: (error) => toast("Selected summaries could not be exported", { description: error.message }) });
  ...
  const toggleArchiveJob = (jobId: number) => setSelectedArchiveJobIds((current) => {
    const next = new Set(current);
    if (next.has(jobId)) { next.delete(jobId); return next; }
    if (next.size >= 10) { toast("You can export up to 10 summaries at a time"); return current; }
    next.add(jobId);
    return next;
  });

  const toggleCurrentArchivePage = () => {
    const visibleJobIds = completionArchive.data?.items.map(({ job }) => job.id) ?? [];
    if (!visibleJobIds.length) return;
    setSelectedArchiveJobIds((current) => {
      const next = toggleCurrentArchivePageSelection(Array.from(current), visibleJobIds);
      if (next.capped) { toast("Select up to 10 summaries per ZIP. Clear some selections before selecting this page."); return current; }
      return new Set(next.selectedIds);
    });
  };
  ...
  return <div className="portal-shell">
    ...
    <main className="customer-portal">
      ...
      {collections.isLoading ? <div className="portal-state"><LoaderCircle className="assessment-spin" size={25} />Opening your collection information</div> : routes.length ? <>
        ...
        <section className="portal-completion-archive" aria-label="Completion summary archive"><div className="portal-section-head"><div><p className="asset-label dark-label"><span className="label-dot" />COMPLETION ARCHIVE</p><h2>Past summaries, <em>ready when needed.</em></h2></div><small>Only completed Core Jobs in your organisation are listed.</small></div><div className="portal-archive-filters"><label>Find a job<input value={archiveSearch} onChange={(event) => { setArchiveSearch(event.target.value); setArchivePage(1); }} placeholder="Reference or title" /></label><label>Completed from<input type="date" value={archiveFrom} onChange={(event) => { setArchiveFrom(event.target.value); setArchivePage(1); }} /></label><label>Completed to<input type="date" value={archiveTo} onChange={(event) => { setArchiveTo(event.target.value); setArchivePage(1); }} /></label><label>Issued document<select value={archiveDocumentType} onChange={(event) => { setArchiveDocumentType(event.target.value as typeof archiveDocumentType); setArchivePage(1); }}><option value="all">All summaries</option><option value="securaze_report">Securaze evidence</option><option value="destruction_certificate">Destruction certificates</option><option value="impact_statement">Impact statements</option></select></label><label>Order<select value={archiveSort} onChange={(event) => { setArchiveSort(event.target.value as "newest" | "oldest"); setArchivePage(1); }}><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></label></div>{completionArchive.data?.items.length ? <label className="portal-select-page"><input type="checkbox" checked={completionArchive.data.items.every(({ job }) => selectedArchiveJobIds.has(job.id))} onChange={toggleCurrentArchivePage} />Select all on this page</label> : null}{selectedArchiveJobIds.size ? <div className="portal-bulk-export"><span>{selectedArchiveJobIds.size} selected <small>Maximum 10 summaries per ZIP · includes completion-summary-manifest.csv</small></span><div><button onClick={() => setSelectedArchiveJobIds(new Set())}>Clear</button><button disabled={selectedArchiveJobIds.size < 2 || bulkCompletionExport.isPending} onClick={() => bulkCompletionExport.mutate({ jobIds: Array.from(selectedArchiveJobIds) })}>{bulkCompletionExport.isPending ? <LoaderCircle className="assessment-spin" size={15} /> : <Download size={15} />}Export ZIP</button></div></div> : null}{completionArchive.isLoading ? <p className="portal-empty-inline"><LoaderCircle className="assessment-spin" size={16} />Loading completed jobs</p> : completionArchive.data?.items.length ? <><div className="portal-archive-list">{completionArchive.data.items.map(({ job, collection }) => <article key={job.id}><label className="portal-archive-select"><input type="checkbox" checked={selectedArchiveJobIds.has(job.id)} onChange={() => toggleArchiveJob(job.id)} /><span className="sr-only">Select {job.jobReference} for bulk export</span></label><div><span>{job.jobReference}</span><h3>{job.title}</h3><p>{collection?.reference ? `Collection ${collection.reference} · ` : ""}Completed {formatDate(job.completedAt)}</p></div><div className="portal-archive-actions"><button disabled={completionSummary.isPending} onClick={() => fetchCompletionSummary(job.id, "preview")}>{completionSummary.isPending ? <LoaderCircle className="assessment-spin" size={15} /> : <FileText size={15} />}Preview</button><button disabled={completionSummary.isPending} onClick={() => fetchCompletionSummary(job.id, "download")}><Download size={15} />Download</button></div></article>)}</div>{completionArchive.data.pageCount > 1 ? <nav className="portal-archive-pagination" aria-label="Completion archive pagination"><button disabled={archivePage === 1} onClick={() => setArchivePage((page) => page - 1)}>Previous</button><span>Page {completionArchive.data.page} of {completionArchive.data.pageCount} · {completionArchive.data.total} summaries</span><button disabled={archivePage === completionArchive.data.pageCount} onClick={() => setArchivePage((page) => page + 1)}>Next</button></nav> : null}</> : <p className="portal-empty-inline">No completed jobs match these archive filters.</p>}</section>

        {session.data.role === "admin" ? <section className="portal-export-audit" aria-label="Bulk export history"><div className="portal-section-head"><div><p className="asset-label dark-label"><span className="label-dot" />CLIENT ADMIN / EXPORT HISTORY</p><h2>Package download <em>history.</em></h2></div><small>Visible to client administrators only.</small></div>{bulkExportAudit.isLoading ? <p className="portal-empty-inline"><LoaderCircle className="assessment-spin" size={16} />Loading export history</p> : bulkExportAudit.data?.length ? <div className="portal-export-audit-list">{bulkExportAudit.data.map(({ event, account }) => <article key={event.id}><div><span>{account.email}</span><h3>{event.summaryCount} completion summaries</h3><p>{event.jobReferences}</p></div><time>{formatDate(event.createdAt)}</time></article>)}</div> : <p className="portal-empty-inline">No bulk packages have been exported by this organisation yet.</p>}</section> : null}
