...
export const appRouter = router({
  ...
  clientPortal: router({
    ...
    completionArchive: clientPortalProcedure.input(z.object({ search: z.string().trim().max(160).optional(), completedFrom: z.coerce.date().optional(), completedTo: z.coerce.date().optional(), documentType: z.enum(["securaze_report", "destruction_certificate", "impact_statement"]).optional(), sort: z.enum(["newest", "oldest"]).default("newest"), page: z.number().int().positive().default(1), pageSize: z.number().int().min(5).max(50).default(10) }).refine((input) => !input.completedFrom || !input.completedTo || input.completedFrom <= input.completedTo, { message: "Completion start date must be on or before end date", path: ["completedTo"] })).query(({ ctx, input }) => listClientPortalCompletionArchive(ctx.clientSession.organisationId, ctx.clientSession.brand, input)),
